...
# The functions below are broken.

def DrawFloorsWalls(console, objects, chart, player):
    fov_map = tcod.map_new(chart.width, chart.height)
    for category in (1, 0):
        for y in range(0, chart.height):
            for x in range(0, chart.width):
...
def DrawObjects(console, objects, chart, player):
    fov_map = tcod.map_new(chart.width, chart.height)
...
def DrawEntities(console, objects, characters, chart, player):
    fov_map = tcod.map_new(chart.width, chart.height)
    grids = []
    for category in range(0, 43):
        for y in range(0, chart.height):
